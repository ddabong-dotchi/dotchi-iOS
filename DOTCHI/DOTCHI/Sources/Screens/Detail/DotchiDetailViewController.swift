...
final class DotchiDetailViewController: BaseViewController {
    ...
    private let navigationView = DotchiNavigationView(type: .closeMore)
    ...
    private let commentButton: DoneButton = {
    ...
    private func setTotalCommentCount() {
        self.totalLuckyLabel.text = Text.total + "\(self.comments.count)"
        self.totalLuckyLabel.setColor(to: "\(self.comments.count)", with: self.luckyType.uiColorNormal())
    }
...
extension DotchiDetailViewController {
    private func fetchData() {
        CardService.shared.getCardDetail(cardId: self.cardId) { networkResult in
            if case .success(let responseData) = networkResult {
                if let result = responseData as? CardDetailResponseDTO {
                    self.user = result.toCardUserEntity()
                    self.luckyType = LuckyType(rawValue: result.type) ?? .lucky
                    self.setData(
                        data: CardEntity(
                            user: self.user,
                            front: result.toCardFrontEntity(),
                            back: result.toCardBackEntity()
                        )
                    )
                    self.getComments()
                }
    ...
    private func getComments() {
        CommentService.shared.getComments(cardId: self.cardId) { networkResult in
            switch networkResult {
            case .success(let responseData):
                if let result = responseData as? GetCommentsResponseDTO {
                    self.comments = result.map({ comment in
    ...
    private func postComment() {
        CommentService.shared.postComment(cardId: self.cardId) { networkResult in
            switch networkResult {
            case .success:
                self.fetchData()
            default:
